    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if args.fidelity_train_en:
            ...
            if 'synthetic' in dataset.source_path:
                gt_folder = '/fs/nexus-projects/dyn3Dscene/Codes/datasets/nerf_synthetic/resized'
                scene_name = os.path.basename(dataset.source_path)
                gt_path = os.path.join(gt_folder, scene_name, f'train_{lr_resolution}', viewpoint_cam.image_name+'.png')
            else:
            ...
            image_gt_lr = Image.open(gt_path).convert('RGB')
...
def train_proposed_2025(dataset, op, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args, dataset2=None):
    ...
    images_path_ori = natsorted(glob.glob(os.path.join(args.init_img, "*")))
    ...
    op.densify_from_iter = args.densify_start
                    ...
                    with precision_scope("cuda"):
                        ...
                        for img_id in range(len(im_path_bs)):
                            ...
                            #############################################
                                ...
                                # print('Finished:', outpath)
                                # import pdb; pdb.set_trace()
                                if 'synthetic' in args.init_img:
                                    gt_path = dataset.source_path
                                    gt_img = np.array(Image.open(os.path.join(gt_path,'train', basename+'.png')).convert("RGBA"))
                                    alpha = gt_img[:,:,-1]                                
                                    img_sr_new = np.zeros((alpha.shape[0], alpha.shape[1], 4))
                                    img_sr_new[:,:,:3] = im_sr[0, ]
                                    img_sr_new[:,:,3] = alpha
                                    out = Image.fromarray(img_sr_new.astype(np.uint8), mode="RGBA")
                                else:
                                    out = Image.fromarray(im_sr[0, ].astype(np.uint8))
                                # out = Image.fromarray(im_sr[0, ].astype(np.uint8))
                                # import pdb; pdb.set_trace()
                                out.save(outpath)
                                print('Finished:', outpath)
...
def parse_args():
    ...
    parser.add_argument("--densify_start", type=int, default=15000)
    #############################################
    #### From Stable SR code ####
    #############################################
    parser.add_argument(
